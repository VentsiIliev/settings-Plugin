...
from src.utils_widgets.SwitchButton import QToggle
...
class CameraControlsWidget(QWidget):
    ...
    def _build_area_row(self) -> QWidget:
        ...
        for area_name, label_text, color in [
            ("pickup_area", "Pickup Area", _AREA_COLORS["pickup_area"]),
            ("spray_area",  "Spray Area",  _AREA_COLORS["spray_area"]),
        ]:
            btn = QPushButton(label_text)
            btn.setCheckable(True)
            btn.setStyleSheet(_area_btn_style(color))
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            btn.clicked.connect(lambda checked, n=area_name: self._on_area_btn(n, checked))
            self._area_btns[area_name] = btn
            hl.addWidget(btn)
    ...
    def _action_btn(label: str, signal: pyqtSignal) -> MaterialButton:
        btn = MaterialButton(label, color="#905BA9")
